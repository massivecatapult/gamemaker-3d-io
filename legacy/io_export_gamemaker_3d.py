...
class Export_gm3d(bpy.types.Operator, ExportHelper):
        ...
        use_alt_export_style = BoolProperty(name="Use alternate script style",
          description="Outputs the model script in a slightly different way, allowing for more easily modular pieces to be added to existing models",
          default=False)
          
        export_style = EnumProperty(
                items=[('DEFAULT', "Default GML script", "A basic text file with a ready-to-use GML script"),
                        ('ALTERNATE', "Versatile GML script", "Similar to the default option, but with added"),
                        ('INTERNAL', "D3D Model", "The internal GameMaker model format")
                        ],
                name="Type:",
                description="Select the style of script to export"
                )
...
def menu_func(self, context):
        self.layout.operator(Export_gm3d.bl_idname, text="GameMaker 3D Model")
